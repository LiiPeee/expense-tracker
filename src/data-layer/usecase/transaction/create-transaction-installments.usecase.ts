...
export class CreateTransactionInstallmentsUseCase implements ICreateTransactionDayUseCase {
  constructor(
  ...
  async execute(input: CreateTransactionInput): Promise<CreateTransactionOutPut> {
    ...
    //     input.recurrence === "M"
    //       ? await this.createInstallments(transaction, new Date(date.setMonth(date.getMonth() + 1)))
